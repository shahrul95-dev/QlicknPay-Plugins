...
class ControllerExtensionPaymentQlicknpay extends Controller
{
    public function index()
    {
        ...
        if (!empty($this->session->data['payment_address']['postcode'])) {
          ...
          $data['Qlicknpay_url'] = 'https://www.demo.Qlicknpay.com/merchant/api/v1/opencart_receiver.php';
        ...
        $data['Qlicknpay_amount'] = str_replace(',', '', $data['Qlicknpay_amount']);
        $data['Qlicknpay_hash'] = md5($data['Qlicknpay_merchant_id'].$this->config->get('Qlicknpay_API').$data['Qlicknpay_detail'].$data['Qlicknpay_amount'].$this->session->data['order_id']);
        ...
        return $this->load->view('extension/payment/Qlicknpay.tpl', $data);
    }
    ...
    public function callback_failed()
    {
        ...
        $this->response->setOutput($this->load->view('extension/payment/Qlicknpay_failed.tpl', $data));
    ...
    public function callback_response()
    {
        if((isset($this->request->post['fpx_fpxTxnId']) || isset($this->request->post['paypal_trx_id']) || isset($this->request->post['mastercard_trx_id']) || isset($this->request->post['others_trx_id'])) && isset($this->request->post['invoice']) && isset($this->request->post['msg']) && isset($this->request->post['hash']))
        {
            ...
            if($order_info)
            {
                if($order_info['order_status_id'] != $this->config->get('Qlicknpay_order_status_id'))
                {
                    $hash_value = md5($this->config->get('Qlicknpay_API').$trx_id.$invoice.$msg);
                    ...
                    if($hash_value === $hash)
                    {
                      echo "OK";
                        ...
                        {
                            $this->model_checkout_order->addOrderHistory($invoice, $this->config->get('Qlicknpay_order_status_id'), 'Payment was made using Qlicknpay through '.$method.'. Qlicknpay transaction id is '.$trx_id, false);
                            if($this->cart->hasProducts())
                                $this->cart->clear();
                        }
                    }
